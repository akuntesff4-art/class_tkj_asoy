...
/* ============================================================
   1. INTRO (cinematic — ring progress, letter reveal, curtain-open)
   Cuma tampil SEKALI di awal, karena sekarang single-page (SPA):
   pindah "halaman" = ganti panel via JS, bukan reload dokumen.
============================================================ */
(function intro() {
  ...
  // huruf judul muncul satu-satu dengan jitter halus supaya kerasa organik, bukan mesin
  if (titleEl) {
    const msg = "CLASS QUESTERS";
    titleEl.innerHTML = msg
      .split("")
      .map((c, i) => {
        const delay = (0.15 + i * 0.045 + Math.random() * 0.03).toFixed(3);
        return `<span style="animation-delay:${delay}s">${c === " " ? "&nbsp;" : c}</span>`;
      })
      .join("");
  }

  const RING_LEN = 326; // sesuai stroke-dasharray di CSS
  let shown = 0, target = 12, finished = false;
  const startedAt = performance.now();
  const MIN_MS = 1300, MAX_MS = 4200;
  ...
  function finish() {
    if (finished) return;
    finished = true;
    screen.classList.add("open"); // curtain slide + stage fade
    setTimeout(() => {
  ...
  function tick(now) {
    ...
    if (ringProgress) ringProgress.style.strokeDashoffset = String(RING_LEN - (Math.min(100, shown) / 100) * RING_LEN);

    const doneEnough = shown >= 99.2 && elapsed >= MIN_MS;
...
/* ============================================================
   2. ROUTER (SPA — ganti panel via hash, tanpa reload dokumen)
   Ini kenapa music player sekarang nggak pernah reset lagi:
   <audio> tidak pernah dibongkar, cuma section yang di-tampil/sembunyikan.
============================================================ */
const PAGE_TITLES = {
  beranda: "CLASS QUESTERS TKJ A 👑⚡",
  struktur: "Struktur Kelas — CLASS QUESTERS TKJ A",
  mapel: "Jadwal Mapel — CLASS QUESTERS TKJ A",
  piket: "Jadwal Piket — CLASS QUESTERS TKJ A",
  gallery: "Gallery — CLASS QUESTERS TKJ A",
  social: "Social Media — CLASS QUESTERS TKJ A",
  about: "About — CLASS QUESTERS TKJ A",
};
let heroEngine = null;

function currentRoute() {
  const hash = (location.hash || "#beranda").replace("#", "");
  return PAGE_TITLES[hash] ? hash : "beranda";
}

function goToRoute(route, options) {
  const opts = options || {};
  document.querySelectorAll(".page-panel").forEach((p) => p.classList.remove("active"));
  const panel = document.getElementById("page-" + route);
  ...
  document.title = PAGE_TITLES[route] || PAGE_TITLES.beranda;
  window.scrollTo({ top: 0, behavior: reduceMotion || opts.instant ? "auto" : "smooth" });

  // hero particle engine cuma jalan pas panel beranda aktif (hemat performa)
  if (heroEngine) {
    if (route === "beranda") heroEngine.start();
    else heroEngine.stop();
  }

  window.ClassSound && ClassSound.play("open");
  document.dispatchEvent(new CustomEvent("cq-route-changed", { detail: { route } }));
}

function initRouter() {
  const canvas = document.getElementById("heroParticles");
  if (canvas && window.ClassParticles) {
    heroEngine = ClassParticles.createEngine(canvas, { density: 16000, maxParticles: 70, emberRatio: 0.14 });
  }
  goToRoute(currentRoute(), { instant: true });
